...
func NewElasticServiceHttpQueryHandler(queryRepository *QueryRepository) (ret *HttpQueryHandler) {
...
func NewElasticServiceHttpCommandHandler(context context.Context, commandBus eventhorizon.CommandHandler) (ret *HttpCommandHandler) {
...
func New@@EMPTY@@(pathPrefix string, context context.Context, commandBus eventhorizon.CommandHandler, 
    ...
    pathPrefix = pathPrefix + "/" + "elasticServices"
    entityFactory := func() eventhorizon.Entity { return NewElasticService() }
    repo := readRepos(string(ElasticServiceAggregateType), entityFactory)
    queryRepository := NewElasticServiceQueryRepository(repo, context)
    queryHandler := NewElasticServiceHttpQueryHandler(queryRepository)
    commandHandler := NewElasticServiceHttpCommandHandler(context, commandBus)
    ret = &Router{
...
type ElasticRouter struct {
    PathPrefix string `json:"pathPrefix" eh:"optional"`
    ElasticServiceRouter *Router `json:"elasticServiceRouter" eh:"optional"`
    Router *mux.Router `json:"router" eh:"optional"`
}

func New@@EMPTY@@(pathPrefix string, context context.Context, commandBus *bus.CommandHandler, 
                readRepos func (string, func () (ret eventhorizon.Entity) ) (ret eventhorizon.ReadWriteRepo) ) (ret *ElasticRouter) {
    pathPrefix = pathPrefix + "/" + "elastic"
    elasticServiceRouter := New@@EMPTY@@(pathPrefix, context, commandBus, readRepos)
    ret = &ElasticRouter{
        PathPrefix: pathPrefix,
        ElasticServiceRouter: elasticServiceRouter,
    }
    return
}

func (o *ElasticRouter) Setup(router *mux.Router) (err error) {
    if err = o.ElasticServiceRouter.Setup(router); err != nil {
        return
    }
    return
}
