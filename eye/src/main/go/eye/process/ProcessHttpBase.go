...
func NewProcessServiceHttpQueryHandler(queryRepository *QueryRepository) (ret *HttpQueryHandler) {
...
func NewProcessServiceHttpCommandHandler(context context.Context, commandBus eventhorizon.CommandHandler) (ret *HttpCommandHandler) {
...
func New@@EMPTY@@(pathPrefix string, context context.Context, commandBus eventhorizon.CommandHandler, 
    ...
    pathPrefix = pathPrefix + "/" + "processServices"
    entityFactory := func() eventhorizon.Entity { return NewProcessService() }
    repo := readRepos(string(ProcessServiceAggregateType), entityFactory)
    queryRepository := NewProcessServiceQueryRepository(repo, context)
    queryHandler := NewProcessServiceHttpQueryHandler(queryRepository)
    commandHandler := NewProcessServiceHttpCommandHandler(context, commandBus)
    ret = &Router{
...
type ProcessRouter struct {
    PathPrefix string `json:"pathPrefix" eh:"optional"`
    ProcessServiceRouter *Router `json:"processServiceRouter" eh:"optional"`
    Router *mux.Router `json:"router" eh:"optional"`
}

func New@@EMPTY@@(pathPrefix string, context context.Context, commandBus *bus.CommandHandler, 
                readRepos func (string, func () (ret eventhorizon.Entity) ) (ret eventhorizon.ReadWriteRepo) ) (ret *ProcessRouter) {
    pathPrefix = pathPrefix + "/" + "process"
    processServiceRouter := New@@EMPTY@@(pathPrefix, context, commandBus, readRepos)
    ret = &ProcessRouter{
        PathPrefix: pathPrefix,
        ProcessServiceRouter: processServiceRouter,
    }
    return
}

func (o *ProcessRouter) Setup(router *mux.Router) (err error) {
    if err = o.ProcessServiceRouter.Setup(router); err != nil {
        return
    }
    return
}
