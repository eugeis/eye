...
func NewMySqlServiceHttpQueryHandler(queryRepository *QueryRepository) (ret *HttpQueryHandler) {
...
func NewMySqlServiceHttpCommandHandler(context context.Context, commandBus eventhorizon.CommandHandler) (ret *HttpCommandHandler) {
...
func New@@EMPTY@@(pathPrefix string, context context.Context, commandBus eventhorizon.CommandHandler, 
    ...
    pathPrefix = pathPrefix + "/" + "mySqlServices"
    entityFactory := func() eventhorizon.Entity { return NewMySqlService() }
    repo := readRepos(string(MySqlServiceAggregateType), entityFactory)
    queryRepository := NewMySqlServiceQueryRepository(repo, context)
    queryHandler := NewMySqlServiceHttpQueryHandler(queryRepository)
    commandHandler := NewMySqlServiceHttpCommandHandler(context, commandBus)
    ret = &Router{
...
type MySqlRouter struct {
    PathPrefix string `json:"pathPrefix" eh:"optional"`
    MySqlServiceRouter *Router `json:"mySqlServiceRouter" eh:"optional"`
    Router *mux.Router `json:"router" eh:"optional"`
}

func New@@EMPTY@@(pathPrefix string, context context.Context, commandBus *bus.CommandHandler, 
                readRepos func (string, func () (ret eventhorizon.Entity) ) (ret eventhorizon.ReadWriteRepo) ) (ret *MySqlRouter) {
    pathPrefix = pathPrefix + "/" + "mySql"
    mySqlServiceRouter := New@@EMPTY@@(pathPrefix, context, commandBus, readRepos)
    ret = &MySqlRouter{
        PathPrefix: pathPrefix,
        MySqlServiceRouter: mySqlServiceRouter,
    }
    return
}

func (o *MySqlRouter) Setup(router *mux.Router) (err error) {
    if err = o.MySqlServiceRouter.Setup(router); err != nil {
        return
    }
    return
}
