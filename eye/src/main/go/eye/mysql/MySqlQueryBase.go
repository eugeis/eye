...
func NewMySqlServiceQueryRepository(repo eventhorizon.ReadRepo, context context.Context) (ret *QueryRepository) {
    ret = &QueryRepository{
        repo: repo,
        context: context,
    }
...
func (o *QueryRepository) FindAll() (ret []*MySqlService, err error) {
    var result []eventhorizon.Entity
	if result, err = o.repo.FindAll(o.context); err == nil {
        ret = make([]*MySqlService, len(result))
		for i, e := range result {
            ret[i] = e.(*MySqlService)
		}
    }
...
func (o *QueryRepository) FindById(id eventhorizon.UUID) (ret *MySqlService, err error) {
    var result eventhorizon.Entity
	if result, err = o.repo.Find(o.context, id); err == nil {
        ret = result.(*MySqlService)
    }
